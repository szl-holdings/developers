...
| SLSA Build L2 provenance (4 public) | `gh attestation verify` | 2 minutes |
...
```bash
# Example: confirm the exact Lean numbers on a running Space
curl -s https://szlholdings-a11oy.hf.space/api/a11oy/v1/honest \
  | jq '{kernel_commit, declarations, axioms_unique, sorries_total}'
# => {"kernel_commit":"c7c0ba17","declarations":749,"axioms_unique":14,"sorries_total":163}
```

---

## 2. SLSA Build L2 provenance — verify via `gh attestation verify`

All five flagship container images have `actions/attest-build-provenance@v2.4.0` wired in their `ghcr-build-push.yml` workflows and have produced Sigstore-signed SLSA Provenance v1 attestations on `runner_environment: github-hosted`.
...
### What SLSA Build L2 means here

Per the [SLSA v1.0 spec](https://slsa.dev/spec/v1.0/levels), L2 requires:
1. Build runs on a **hosted platform** (GitHub Actions `ubuntu-latest`) ✅
2. **Signed provenance** generated by the build service (Fulcio OIDC short-lived cert, Rekor entry) ✅
3. Provenance ties the artifact digest to the source commit and workflow ✅

The predicate type is `https://slsa.dev/provenance/v1`. The Fulcio certificate SAN for each image is `https://github.com/szl-holdings/REPO/.github/workflows/ghcr-build-push.yml@refs/heads/main`.

**What is NOT claimed:** SLSA L3 (requires signing in an isolated reusable workflow — not our current setup).
...
# UDS bundle repo: https://github.com/szl-holdings/uds-mesh
```
...
Each component image has its own SLSA L2 attestation verifiable above.
...
# Confirm receipt invariant: receipts.in ≡ receipts.out
curl -s https://szlholdings-a11oy.hf.space/api/a11oy/v1/honest | jq .receipt_invariant
